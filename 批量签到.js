...
function SignIn_Baidu_Wenku() {
    try {
        if (text_log) {
            var patter = text_log.search("百度文库");
            if (patter != -1) {
                toastLog("百度文库跳过");
                return;
            }
...
function SignIn_Baidu_netdisk() {
    try {
        ...
        if (我的) {
            var 我的 = 我的.bounds();
            click(我的.centerX(), 我的.centerY());
            var 待领取 = textContains("待领取").findOne(5000);
            if (待领取) {
                var 待领取 = 待领取.bounds();
...
function SignIn_Fandengreader() {
    try {
        ...
        while (我的) {
            ...
            click(我的.centerX(), 我的.centerY());
            sleep(1000);
            var 签到有礼 = text("签到有礼").findOne(2000);
            if (签到有礼) {
...
function SignIn_Netease_Cloudmusic() {
    try {
        ...
        var 每日推荐 = text("每日推荐").findOne(10000);
        if (每日推荐) {
            sleep(1000);
            click(100, 150);//打开菜单栏
            sleep(1000);
