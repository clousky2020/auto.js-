        ...
        if (任务界面) {
            ...
            while (num_limit < 2) {
                ...
                while (1) {
                    if (任务界面) {
                        toastLog("进入任务界面");
                        loop_find_click(text, '领取奖励', 500, 4);
                        list_1 = ["去完成", "去浏览", ];
                        for (var i = 0; i < list_1.length; i++) {
                            var click_title = text(list_1[i]).findOne(1500);
                            if (x != 0) {
                                var click_title = text(list_1[i]).findOnce(x);
                            }
                            if (click_title) {
                                // sleep(1000);
                                var state = 0;
                                //有部分内容不需要进入，直接跳过
                                var head_text = click_title.parent().children()[0].children()[0].text();
                                var skip_texts = ['加油', '下单', '买特', '消消乐', '话费', '天猫', '桌面', '菜鸟裹裹', '车险', '夺宝', '淘金币免费', '点击商品图片', '逛淘金币新商家专场', '小说', '金币红包赛', '618'];
...
function coin_loop() {
    ...
    if (descContains("添加").findOne(1000)) {
        ...
        toastLog("没有找到设定内容，等待10秒")
        sleep(10000);
    }
    while (1) {
        if (text("今日总能量").findOne(4000)) {
            break;
        } else {
            toastLog("还未退回任务界面！");
            back();
